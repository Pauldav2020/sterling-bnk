// Fiserv Plugins 1.1.0
//Pulled 7/9/2018
//Changed Scroll To 1.2.0
...
// Scroll To 1.2.0 (c) Jesse Fowler, Fiserv 2015.  All rights reserved.
// Requires jQuery and CSS.
(function (jQuery) {
    jQuery.fn.scrollTo = function (options) {
        var settings = jQuery.extend({
            skipInitial: 0,
            scrollToElementsClassName: 'scroll-to', // Required to know which elements should be reset as a whole.
            scrollToCurrentElementClassName: 'scroll-to-current', // Required to note which item is currently scrolled to.
            trigger: jQuery(".scroll-trigger"), // The selector for which element triggers the page to scroll down i.e. a down arrow.
            activeTriggerClassName: "active", // Use to style the active link when combined with a tabbed list of trigger links.
            baseOffsetTopObject: jQuery('header').eq(0), // Fixed object that overlays the scrollable content area.
            additionalOffsetTop: 0 // Added offset height to the scroll baseOffsetTopObject
        }, options);
        var $this = this,
            currentElement = 0 + settings.skipInitial,
            baseOffsetTopObjectValue = 0;

        jQuery(this).addClass(settings.scrollToElementsClassName);
        jQuery(this).eq(0).addClass(settings.scrollToCurrentElementClassName);

        if (this.length > 0 + settings.skipInitial) {
            settings.trigger.addClass(settings.activeTriggerClassName);
            ...
            settings.trigger.click(function () {
                ...
                $this.each(function (index) {
                    if (jQuery(this).is(jQuery('.' + settings.scrollToCurrentElementClassName).eq(0))) {
                ...
                if ($this.eq(currentElement).offset().top != scrollPos) {
                    baseOffsetTopObjectValue = settings.baseOffsetTopObject.length > 0 ? settings.baseOffsetTopObject.height() : 0;
                    var scrollTopGoal = $this.eq(currentElement).offset().top - (parseInt(baseOffsetTopObjectValue) + parseInt(settings.additionalOffsetTop));
                    jQuery('html, body').animate({
                        scrollTop: scrollTopGoal
                    }, 2000);
                    jQuery('.' + settings.scrollToCurrentElementClassName).removeClass(settings.scrollToCurrentElementClassName);
                    $this.eq(currentElement).addClass(settings.scrollToCurrentElementClassName);
                }
